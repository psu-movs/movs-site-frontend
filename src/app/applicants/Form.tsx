...
export default function Applicants({companies}: ApplicantsProps) {
  ...
  return (
    <Box 
      sx={{ 
        margin: isPhone ? "0% 5% 4% 5%" :  isTablet ? "0% 10% 4% 10%" : "0% 20% 4% 20%", 
        maxWidth: '100%',
        display: "flex", 
    ...
    >
      <Stack mb={ isPhone ? "10%" : "5%"}>
        ...
        <Typography color={"#4C4A53"} sx={{fontSize: isPhone ? 16 : 24, fontWeight: "medium"}}>
          Наши выпускники работают не только в городах России, но и по всему миру!
        </Typography>
      </Stack>
      <Grid container spacing={3} sx={{display: "flex", justifyContent: "center", mb: isPhone ? "15%" : "8%"}} columns={gridColumns} >
      ...
      <Box display={"flex"} justifyContent={"center"} mb={"15%"}>
        <Divider sx={{width: "60%", border: "1px solid #2148C0", opacity: "75%"}} />
      </Box>
    </Box>
  );
}
